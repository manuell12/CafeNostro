...
class FormularioProducto(QtGui.QDialog):
    ...
    def connect_actions(self):
        ...
        self.ui.lineEdit_precio_neto.textChanged.connect(self.lineEdit_precio_neto_changed)
        self.ui.comboBox_id_categoria.currentIndexChanged.connect(self.comboBox_id_categoria_changed)

    def lineEdit_nombre_changed(self,text):
        controller_admin_producto.validarNombreF(self.ui.label_error_nombre,text)

    def lineEdit_precio_neto_changed(self,text):
        controller_admin_producto.validarPrecioNetoF(self.ui.label_error_precio,text)

    def comboBox_id_categoria_changed(self,index):
        if(index != 0):
            self.ui.label_error_categoria.setText(u"<font color='green'><b>Seleccion correcta.</b></font>")
        else:
            self.ui.label_error_categoria.setText(u"<font color='red'><b>Debe seleccionar un tipo.</b></font>")
