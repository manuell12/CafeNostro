...
class Estadistica(QtGui.QWidget):
    """
    Clase que genera un widget donde se muestran graficos y 
    tablas con estadisticas sobre el local.
    """
    ...
    def crearymostrar_html(self):
        ...
        file.write(controller.crear_html(
            self.lista_ProductoVenta,fecha_inicio,fecha_fin).encode('utf-8'))
        file.close()

        self.ui.webView.load(QtCore.QUrl(
            "file:///"+os.getcwd()+"/estadisticas/charts.htm"))
        self.ui.webView.show()

    def connect_actions(self):
        """
        Método que accede a los slots de los widgets de la interfaz 
        gráfica y los conecta con metodos de la clase Estadistica
        """
        self.ui.calendarWidget_inicio.selectionChanged.connect(
            self.actualizar_productos)
        self.ui.calendarWidget_fin.selectionChanged.connect(
            self.actualizar_productos)
    ...
    def load_model_total_productos(self, data=""):
        """
        Método que carga en la tabla 'tableView' la información pasada en el 
        atributo 'data', debe ser una lista de objetos
        """
    ...
    def set_columns_total_productos(self):
        """
        Método que ajusta el tamaño de las columnas de la tabla al 
        especificado en '__header_table__'
        """
